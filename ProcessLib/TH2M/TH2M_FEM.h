...
//            }
  //      }
                              ...
                              std::vector<double>& local_Jac_data) override
    {
        ...
        PPMatrix Mgpg =
        		PPMatrix::Zero(gas_pressure_size, gas_pressure_size);
        PPMatrix Mgpc =
        		PPMatrix::Zero(gas_pressure_size,cap_pressure_size);
        PPMatrix MgT =
        		PPMatrix::Zero(gas_pressure_size,temperature_size);
        PUMatrix Mgus =
        		PUMatrix::Zero(gas_pressure_size, displacement_size);

        PPMatrix Mlpg =
        		PPMatrix::Zero(cap_pressure_size, gas_pressure_size);
        PPMatrix Mlpc =
        		PPMatrix::Zero(cap_pressure_size,cap_pressure_size);
        PPMatrix MlT =
        		PPMatrix::Zero(cap_pressure_size,temperature_size);
        PUMatrix Mlus =
        		PUMatrix::Zero(cap_pressure_size, displacement_size);

        PPMatrix Mepg =
        		PPMatrix::Zero(temperature_size,gas_pressure_size);
        PPMatrix Mepc =
        		PPMatrix::Zero(temperature_size,cap_pressure_size);
        PPMatrix MeT =
        		PPMatrix::Zero(temperature_size,temperature_size);
        ...
        // stiffness matrices
        UPMatrix Kupg =
        		UPMatrix::Zero(displacement_size,gas_pressure_size);
        UPMatrix Kupc =
        		UPMatrix::Zero(displacement_size,cap_pressure_size);


        // right hand side vectors
        PVector fg = PVector::Zero(gas_pressure_size);
        PVector fl = PVector::Zero(cap_pressure_size);
        PVector fe = PVector::Zero(temperature_size);
        PVector fu = PVector::Zero(displacement_size);

        // right hand side derivatives
        PVector dfg_dpg = PVector::Zero(gas_pressure_size);
        PVector dfg_dpc = PVector::Zero(cap_pressure_size);
        PVector dfg_dT = PVector::Zero(gas_pressure_size);
        PVector dfg_du = PVector::Zero(gas_pressure_size);

        PVector dfl_dpg = PVector::Zero(gas_pressure_size);
        PVector dfl_dpc = PVector::Zero(gas_pressure_size);
        PVector dfl_dT = PVector::Zero(gas_pressure_size);
        PVector dfl_du = PVector::Zero(gas_pressure_size);

        PVector dfe_dpg = PVector::Zero(gas_pressure_size);
        PVector dfe_dpc = PVector::Zero(gas_pressure_size);
        PVector dfe_dT = PVector::Zero(gas_pressure_size);
        PVector dfe_du = PVector::Zero(gas_pressure_size);

        UVector dfu_dpg = PVector::Zero(gas_pressure_size);
        UVector dfu_dpc = PVector::Zero(gas_pressure_size);
        UVector dfu_dT = PVector::Zero(gas_pressure_size);
        UVector dfu_du = PVector::Zero(gas_pressure_size);
...
//              variables)[1];
            ...
            const double lambda_eff = s_G*phi * rho_GR * lambda_G +
                                      s_L*phi * rho_LR * lambda_L +
                                      phi_S * rho_SR * lambda_S;
...
//                            ( dNdx_p * p_LR + rho_LR * b)).eval();
            ...
            dMgpg_dpg += s_G * drhoGRdpGR * (phi*beta_p_GR + Sps)
                            * mass_operator;
            ...
            dMgpg_dpc -= dsLdpc * rho_GR * (phi*beta_p_GR + Sps)
                    * mass_operator;
            ...
            Mlpc.noalias() +=
            		N_p.transpose() * rho_LR *
					(dsLdpc * (phi - Sps*p_cap) - s_L*(phi*beta_p_LR + Sps)) *  N_p * w;
            ...
            Llpc.noalias() -= rho_LR * k_over_mu_LR * Laplace;
