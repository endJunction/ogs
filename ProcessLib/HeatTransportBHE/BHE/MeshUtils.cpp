    ...
    std::vector<std::vector<MeshLib::Element*>>& vec_BHE_soil_elements)
{
    ...
    for (unsigned bhe_id = 0; bhe_id < vec_BHE_soil_elements.size(); bhe_id++)
    {
        std::vector<MeshLib::Element*>& vec_elements =
            vec_BHE_soil_elements[bhe_id];
    ...
    for (unsigned bhe_id = 0; bhe_id < vec_BHE_mat_IDs.size(); bhe_id++)
    {
        ...
        DBUG("-> found %d nodes connected with BHE_%d", vec_nodes.size(),
             bhe_id);
    }
    ...
    for (unsigned bhe_id = 0; bhe_id < vec_BHE_nodes.size(); bhe_id++)
    {
        std::vector<MeshLib::Node*>& tmp_vec_BHE_nodes = vec_BHE_nodes[bhe_id];
