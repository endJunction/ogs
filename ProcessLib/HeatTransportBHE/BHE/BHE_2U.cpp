...
void ProcessLib::HeatTransportBHE::BHE::BHE_2U::setRMatrices(
        ...
        R_s_matrix) const
{
    ...
    {
        case 0:  // R i1 i2
            R_matrix.block(0, 4 * NumNodes, NumNodes, NumNodes) +=
                -1.0 * matBHE_loc_R;
            R_matrix.block(4 * NumNodes, 0, NumNodes, NumNodes) +=
                -1.0 * matBHE_loc_R;
            R_matrix.block(NumNodes, 5 * NumNodes, NumNodes, NumNodes) +=
                -1.0 * matBHE_loc_R;
            R_matrix.block(5 * NumNodes, NumNodes, NumNodes, NumNodes) +=
                -1.0 * matBHE_loc_R;
            ...
            R_matrix.block(4 * NumNodes,
                           4 * NumNodes,
                           NumNodes,
                           NumNodes) += 1.0 * matBHE_loc_R;  // K_ig
            R_matrix.block(5 * NumNodes,
                           5 * NumNodes,
                           NumNodes,
                           NumNodes) += 1.0 * matBHE_loc_R;  // K_ig
            break;
        case 1:  // R o1 o2
            R_matrix.block(2 * NumNodes, 6 * NumNodes, NumNodes, NumNodes) +=
                -1.0 * matBHE_loc_R;
            R_matrix.block(6 * NumNodes, 2 * NumNodes, NumNodes, NumNodes) +=
                -1.0 * matBHE_loc_R;
            R_matrix.block(3 * NumNodes, 7 * NumNodes, NumNodes, NumNodes) +=
                -1.0 * matBHE_loc_R;
            R_matrix.block(7 * NumNodes, 3 * NumNodes, NumNodes, NumNodes) +=
                -1.0 * matBHE_loc_R;

            R_matrix.block(2 * NumNodes,
                           2 * NumNodes,
                           NumNodes,
                           NumNodes) += 1.0 * matBHE_loc_R;  // K_o1
            R_matrix.block(3 * NumNodes,
                           3 * NumNodes,
                           NumNodes,
                           NumNodes) += 1.0 * matBHE_loc_R;  // K_o2
            R_matrix.block(6 * NumNodes,
                           6 * NumNodes,
                           NumNodes,
                           NumNodes) += 1.0 * matBHE_loc_R;  // K_og
            R_matrix.block(7 * NumNodes,
                           7 * NumNodes,
                           NumNodes,
                           NumNodes) += 1.0 * matBHE_loc_R;  // K_og
            break;
        case 2:  // R g1
            R_matrix.block(4 * NumNodes, 6 * NumNodes, NumNodes, NumNodes) +=
                -1.0 * matBHE_loc_R;
            R_matrix.block(6 * NumNodes, 4 * NumNodes, NumNodes, NumNodes) +=
                -1.0 * matBHE_loc_R;
            R_matrix.block(4 * NumNodes, 7 * NumNodes, NumNodes, NumNodes) +=
                -1.0 * matBHE_loc_R;
            R_matrix.block(7 * NumNodes, 4 * NumNodes, NumNodes, NumNodes) +=
                -1.0 * matBHE_loc_R;
            R_matrix.block(5 * NumNodes, 6 * NumNodes, NumNodes, NumNodes) +=
                -1.0 * matBHE_loc_R;
            R_matrix.block(6 * NumNodes, 5 * NumNodes, NumNodes, NumNodes) +=
                -1.0 * matBHE_loc_R;
            R_matrix.block(5 * NumNodes, 7 * NumNodes, NumNodes, NumNodes) +=
                -1.0 * matBHE_loc_R;
            R_matrix.block(7 * NumNodes, 5 * NumNodes, NumNodes, NumNodes) +=
                -1.0 * matBHE_loc_R;

            R_matrix.block(4 * NumNodes,
                           4 * NumNodes,
                           NumNodes,
                           NumNodes) += 2.0 * matBHE_loc_R;  // K_ig
            R_matrix.block(5 * NumNodes,
                           5 * NumNodes,
                           NumNodes,
                           NumNodes) += 2.0 * matBHE_loc_R;  // K_ig
            R_matrix.block(6 * NumNodes,
                           6 * NumNodes,
                           NumNodes,
                           NumNodes) += 2.0 * matBHE_loc_R;  // K_og
            R_matrix.block(7 * NumNodes,
                           7 * NumNodes,
                           NumNodes,
                           NumNodes) += 2.0 * matBHE_loc_R;  // K_og
            break;
        case 3:  // R g2
            R_matrix.block(6 * NumNodes, 7 * NumNodes, NumNodes, NumNodes) +=
                -1.0 * matBHE_loc_R;
            R_matrix.block(7 * NumNodes, 6 * NumNodes, NumNodes, NumNodes) +=
                -1.0 * matBHE_loc_R;
        ...
        case 4:  // R s
            ...
            R_pi_s_matrix.block(4 * NumNodes, 0, NumNodes, NumNodes) +=
                -1.0 * matBHE_loc_R;
            R_pi_s_matrix.block(5 * NumNodes, 0, NumNodes, NumNodes) +=
                -1.0 * matBHE_loc_R;
            R_pi_s_matrix.block(6 * NumNodes, 0, NumNodes, NumNodes) +=
                -1.0 * matBHE_loc_R;
            R_pi_s_matrix.block(7 * NumNodes, 0, NumNodes, NumNodes) +=
                -1.0 * matBHE_loc_R;

            R_matrix.block(4 * NumNodes, 4 * NumNodes, NumNodes,
                           NumNodes) += matBHE_loc_R;  // K_gs
            R_matrix.block(5 * NumNodes, 5 * NumNodes, NumNodes,
                           NumNodes) += matBHE_loc_R;  // K_gs
            R_matrix.block(6 * NumNodes, 6 * NumNodes, NumNodes,
                           NumNodes) += matBHE_loc_R;  // K_gs
            R_matrix.block(7 * NumNodes, 7 * NumNodes, NumNodes,
                           NumNodes) += matBHE_loc_R;  // K_gs
            break;
    }
}
