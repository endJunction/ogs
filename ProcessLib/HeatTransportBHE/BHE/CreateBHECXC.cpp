...

{
    ...
    const double bhe_inner_pipe_wall_thermal_conductivity =
        bhe_conf.getConfigParameter<double>(
            "bhe_inner_pipe_wall_thermal_conductivity");
    const double bhe_outer_pipe_wall_thermal_conductivity =
        bhe_conf.getConfigParameter<double>(
            "bhe_outer_pipe_wall_thermal_conductivity");
    ...
    else if (bhe_bound_type_str.compare("FIXED_TEMP_DIFF") == 0)
    {
        ...
        bhe_delta_T_val =
            bhe_conf
                .getConfigParameterOptional<double>("bhe_inout_delta_T_value")
                .get();
    }
    else
    {
    ...
    BHE::BHE_CXC* m_bhe_CXC = new BHE::BHE_CXC(
        ...
        {bhe_pipe_inner_radius, bhe_pipe_outer_radius,
         ...
         bhe_inner_pipe_wall_thermal_conductivity, /* For CXC type BHE, used
                                                      inner pipe lambda for wall
                                                      lambda */
         bhe_inner_pipe_wall_thermal_conductivity,
