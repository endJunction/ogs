 ...
 */
std::pair<double, double> BHE_CXA::calcRenoldsNumber() const
{
    double const& r_outer = pipe_param.r_outer;
    double const& r_inner = pipe_param.r_inner;
    double const& b_in = pipe_param.b_in;
    ...
    double const Re_o1 = reynoldsNumber(_u(1), d_o1, mu_r, rho_r);
    double const Re_i1 = reynoldsNumber(_u(0), d_h, mu_r, rho_r);
    return {Re_o1, Re_i1};
}
