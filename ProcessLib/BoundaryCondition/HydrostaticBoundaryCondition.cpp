...
{
void HydrostaticBoundaryCondition::preTimestep(const double /*t*/)
{
    if (_parameter.isTimeDependent())
        _already_computed = false;
}

void HydrostaticBoundaryCondition::getEssentialBCValues(
    const double t, NumLib::IndexValueVector<GlobalIndexType>& bc_values) const
{
    ...
    for (auto const id : _mesh_node_ids)
    {
        ...
        // and MatZeroRowsColumns, which are called to apply the Hydrostatic BC,
        ...
        if (g_idx >= 0) {
            ...
            bc_values.values.emplace_back(_parameter(t, pos).front() -
                                          _alpha * (*_mesh.getNode(id))[2]);
        }
    }
}

std::unique_ptr<HydrostaticBoundaryCondition>
createHydrostaticBoundaryCondition(
...
{
    DBUG("Constructing HydrostaticBoundaryCondition from config.");
    //! \ogs_file_param{prj__process_variables__process_variable__boundary_conditions__boundary_condition__type}
    config.checkConfigParameter("type", "Hydrostatic");

    auto const alpha = config.getConfigParameter<double>("alpha");
    //! \ogs_file_param{prj__process_variables__process_variable__boundary_conditions__boundary_condition__Hydrostatic__parameter}
    auto const param_name = config.getConfigParameter<std::string>("parameter");
    DBUG("Using parameter %s", param_name.c_str());

    auto& param = findParameter<double>(param_name, parameters, 1);

    return std::unique_ptr<HydrostaticBoundaryCondition>(
        new HydrostaticBoundaryCondition(alpha, param, mesh, std::move(mesh_node_ids),
