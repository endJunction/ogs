...
public:
    ...
    void setSigma(double const* values)
    {
        ...
        auto sigma_values =
            Eigen::Map<Eigen::Matrix<double, Eigen::Dynamic, kelvin_vector_size,
                                     Eigen::RowMajor> const>(
                values, n_integration_points, kelvin_vector_size);

        // TODO make a general implementation for converting KelvinVectors
        // from and to symmetric rank-2 tensors.
        for (unsigned ip = 0; ip < n_integration_points; ++ip)
        {
            auto& sigma = _ip_data[ip].sigma;
